...
public class FinUtil {
	 ...
	 *            дата.
	 * @param rate
	 *            ставка кредита в долях от единицы.
	 * @param monthlyCommission
	 *            ежемесячная комиссия.
	 * @return упорядоченный по дате список платежей по кредиту.
	 */
	static List<CreditPayment> calcAnnuityPayments(
			final BigDecimal creditAmount, final int durationInMonths,
			Date date, final BigDecimal rate, final BigDecimal monthlyCommission) {
		ArrayList<CreditPayment> payments = new ArrayList<CreditPayment>();
		// Формула расчета платежа по аннуитетной схеме A = (L / 12) / (1 - (1 +
		// (l / 12)) ^ -n )
		// месячная ставка по кредиту L / 12
		BigDecimal monthlyRate = rate.divide(new BigDecimal(
				CalendarUtil.NUMBER_OF_MONTHS), Constants.CALC_SCALE,
				Constants.ROUNDING_MODE);

		// 1 + (l / 12) ^ -n
		BigDecimal pow = monthlyRate.add(new BigDecimal(1)).pow(
				-durationInMonths, MathContext.DECIMAL64);
		...
		for (int i = 0; i < durationInMonths; i++) {
			...
			// Создаем объект платежа по кредиту с суммой платежа с учетом
			// комисии, если она существует.
			CreditPayment payment = new CreditPaymentImpl(
					withCommAmount.setScale(Constants.CALC_SCALE,
							Constants.ROUNDING_MODE), date);

			// Добавляем и округляем Основной долг, разница между суммой платежа
			...
			payment.setInterest(interest.setScale(Constants.CALC_SCALE,
					Constants.ROUNDING_MODE));
			...
			payment.setTotalLeft(base.setScale(Constants.CALC_SCALE,
					Constants.ROUNDING_MODE));
	 ...
	 * @return упорядоченный по дате список платежей по кредиту.
	 */
	static List<CreditPayment> calcDifferentialPayments(
			final BigDecimal creditAmount, final int durationInMonths,
			Date date, final BigDecimal rate, final BigDecimal monthlyCommission) {
		...
		for (int k = 1; k <= durationInMonths; k++) {
			...
			// (1 / n + L / 12) * S
			BigDecimal firstSum = new BigDecimal(1).divide(durationInMonthsBD,
					Constants.CALC_SCALE, Constants.ROUNDING_MODE);
			...
			BigDecimal secondSum = creditAmount
					...
					.divide(durationInMonthsBD, Constants.CALC_SCALE,
							Constants.ROUNDING_MODE)
			...
			payment.setDebt(creditAmount.divide(durationInMonthsBD,
					Constants.CALC_SCALE, Constants.ROUNDING_MODE).setScale(
					Constants.OUTPUT_AMOUNT_SCALE, Constants.ROUNDING_MODE));
			// вычитаем от остатка размер платежа
			base = base.subtract(amount);

			// процентная часть платежа
			payment.setInterest(interest.setScale(
					Constants.OUTPUT_AMOUNT_SCALE, Constants.ROUNDING_MODE));
			// остаток на следующий месяц
			payment.setTotalLeft(base.setScale(Constants.OUTPUT_AMOUNT_SCALE,
					Constants.ROUNDING_MODE));
	...
	static BigDecimal calcTotalAmount(final List<CreditPayment> payments) {
		BigDecimal totalAmount = new BigDecimal(0).setScale(
				Constants.CALC_SCALE, Constants.ROUNDING_MODE);
	...
	static BigDecimal calcMonthlyCommission(final BigDecimal creditAmount,
		...
		BigDecimal monComm = new BigDecimal(0).setScale(Constants.CALC_SCALE,
				Constants.ROUNDING_MODE);
	...
	public static BigDecimal calcEffectiveRate(CreditProposal proposal) {
		...
		while (currentEps > eps) {
			...
			for (int i = 1; i <= paymentCount; i++) {
				...
				dfx += i * payment.getAmount().doubleValue()
						* Math.pow(x, i - 1);
			}
